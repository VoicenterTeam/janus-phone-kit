...
export class WhiteBoardPlugin extends ConferencingBasePlugin {
  ...
  static getAspectRatioDimensions (stream: MediaStream, wrapperEl: HTMLElement) {
    ...
    const wrapperWidth = wrapperEl.clientWidth //wrapperEl.offsetWidth;
    const wrapperHeight = wrapperEl.clientHeight //wrapperEl.offsetHeight;
  ...
  static async startScreenShareWhiteboard(stream) {
    ...
    //let width = streamSettings.width; //wrapperEl.clientWidth;
    //let height = streamSettings.height; //wrapperEl.clientHeight - 55;//25;
    const {width, height} = this.getAspectRatioDimensions(stream, wrapperEl)
    let shareWidth = width, shareHeight = height
    // first we need Konva core things: stage and layer

    // TODO resize on screen resize as well
  ...
  static stopScreenShareWhiteboard() {
    console.log('stopScreenShareWhiteboard initialStream', this.initialStream)
    console.log('stopScreenShareWhiteboard video', this.video)
    console.log('stopScreenShareWhiteboard wrapperEl', this.wrapperEl)
  ...
  drawEmptyWhiteboard() {
    console.log('drawEmptyWhiteboard')
  ...
  async drawImageWhiteboard() {
    ...
    console.log('drawImageWhiteboard')
    const wrapperEl = document.getElementById('presentation-video-container')
    ...
    let konvaImage = new Konva.Image({
      image: imageObj,
      x: (stage.width() - imageObj.width) / 2, // Center horizontally
      y: (stage.height() - imageObj.height) / 2, // Center vertically
    });

    // Add the image to the layer
    layer.add(konvaImage);

    // Now, your image is centered on the canvas
    // You can add your drawing logic here
    // ...

    // Don't forget to redraw the layer to see the changes
    layer.batchDraw();
  ...
  async onAttached() {
    console.log('onAttached this.mode', this.mode)
